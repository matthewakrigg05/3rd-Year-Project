"""
Runs all four sentiment models against the cleaned tweets dataset.

Loads cleaned_tweets.csv, prompts the user to choose which models to run,
and saves results to classified_tweets.csv and model_run_time.csv.
"""
...
# Configure logging, more info than printing my own messages
logging.basicConfig(
    level=logging.INFO,
...
class TweetClassifier:
    ...
    def __init__(self, input_csv: str = "cleaned_tweets.csv", 
        ...
        """
        Initialise the classifier with input/output paths.
        """
    ...
    def initialise_models(self) -> None:
        """Initialise all sentiment analysis models."""
        ...
        try:
            ...
            logger.info(f"Initialised {len(self.models)} models: {', '.join(self.models.keys())}")
        except Exception as e:
            logger.error(f"Error initialising models: {e}")
            raise
    ...
    def classify_batch(self, model_name: str, model, 
        ...
        """Run all tweets through a single model and return results, elapsed time, and device info."""
    ...
    def run_selected_models(self) -> None:
        ...
        # Initialise results structure for each selected model
    ...
    def save_timing_data(self) -> None:
        """Append timing and performance data to model_run_time.csv."""
    ...
    def run(self) -> None:
        ...
        try:
            # Load tweets and initialise models
            self.load_tweets()
